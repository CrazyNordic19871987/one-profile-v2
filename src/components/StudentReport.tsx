...
export function StudentReport({ student, observations, competencyScores, disc, professions, level, onClose }: StudentReportProps) {
  ...
  return (
    <div className="report-overlay fixed inset-0 z-[5000] overflow-y-auto" style={{ background: 'rgba(19, 34, 69, 0.95)' }}>
      {/* Toolbar */}
      <div className="report-toolbar sticky top-0 z-10 px-4 py-3 flex items-center justify-between no-print" style={{ background: 'rgba(19, 34, 69, 0.95)', borderBottom: '1px solid var(--color-border)' }}>
        <h2 className="text-sm font-bold" style={{ color: 'var(--color-accent)' }}>📄 {t('reportTitle')}</h2>
        <div className="flex gap-2">
          <button onClick={handlePrint} className="btn-accent px-4 py-2 rounded-lg text-sm font-bold">
            🖨️ {t('reportPrint')}
          </button>
      ...
      <div className="report-content max-w-[900px] mx-auto p-6 space-y-6">
        ...
        {/* Header */}
        <div className="text-center py-6" style={{ borderBottom: '3px solid var(--color-accent)' }}>
        ...
        <div className="rp-section flex items-center gap-6 p-6 rounded-xl" style={{ background: '#f8fafc', border: '2px solid #e2e8f0' }}>
          <div className="w-20 h-20 rounded-full flex items-center justify-center text-4xl" style={{ background: 'var(--color-accent-dim)' }}>
            {student.emoji || '🦊'}
          </div>
          <div className="flex-1">
            <h2 className="text-xl font-bold" style={{ color: '#0A0E1A' }}>{student.nickname}</h2>
            <div className="flex gap-3 mt-1">
              <span className="text-sm px-2 py-0.5 rounded-full" style={{ background: 'var(--color-accent-dim)', color: 'var(--color-accent)' }}>
          ...
          <div className="text-right">
            <div className="text-3xl font-bold" style={{ color: 'var(--color-accent)' }}>Lv.{level.level}</div>
          </div>
        </div>
        ...
        <div className="rp-section grid grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="p-3 rounded-lg text-center stat-block">
              <div className="text-lg">{stat.icon}</div>
              <div className="text-xl font-bold stat-value" style={{ color: '#0A0E1A' }}>{stat.value}</div>
              <div className="text-xs stat-label" style={{ color: '#666' }}>{stat.label}</div>
            </div>
          ))}
        </div>
